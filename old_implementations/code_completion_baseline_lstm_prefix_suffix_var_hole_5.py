...
class Code_Completion_Baseline:
    ...
    max_hole_size = 1  #Indicates the dimension of the y vector (label)
    ...
    def prepare_data(self, token_lists):
        ...
        for token_list in token_lists:
            ...
            for idx, token in enumerate(token_list):
                ...
                if has_token_list_enough_prefixes and has_token_list_enough_suffixes:
                    ...
                    for i in range(idx + 1, idx + 1 + self.nr_considered_words_suffix):
                        i_token_string = self.token_to_string(token_list[i])
                        x [row_idx,:] = self.one_hot(i_token_string)
                        row_idx += 1
        ...
        final_ys = numpy.zeros((samples , len(self.string_to_number) * self.y_dim))
    ...
    def create_network(self):
        ...
        self.model.add(Dense(len(self.string_to_number), activation='tanh'))
